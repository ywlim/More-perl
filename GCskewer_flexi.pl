#!/usr/bin/perl
# GCskewer_flexi.pl by Yoong Wearn Lim
# modified from GoodGCskewer_FAlite.pl on 6/19/12 to make it more flexible
# user can determine window size
...
die "usage: GCskewer_flexi.pl <fasta> <strand> <window>\n" unless @ARGV == 3;
...
while (my $entry = $fasta->nextEntry)
{
	...
	for (my $k = 0; $k < $length - ($window + 1); $k++)					# loop each nucleotide (sliding window)
	{
		...
		elsif ($strand eq "+")	{$skew[$w][$k] = ($G - $C)/($G + $C)}
		elsif ($strand eq "-")	{$skew[$w][$k] = ($C - $G)/($G + $C)}
	}
...
for (my $k = 0; $k < $length - ($window + 1); $k++)						# loop each chunk
{
	...
	print $k + ($window / 2),"	$aveskew\n";
}

print "number of genes: ", $w, "\n";
print "length of your sequence: ", $length, "\n";
